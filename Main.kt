...
class NearestNeighbors() {
    ...
    fun validateFloatInput(input: String): Boolean {
        var results = input.toFloatOrNull() != null
        if (results == true){
            if (input.toFloat() < 0){
                results = false
            }
    ...
    fun validateIntInput(input: String): Boolean {
        var results = input.toIntOrNull() != null
        if (results == true){
            if (input.toInt() < 0){
                results = false
            }
    ...
    fun test(numNeighbors : Int, SepalLengthCm: Float, SepalWidthCm: Float, PetalLengthCm: Float, PetalWidthCm: Float): String {
        ...
        for (flower: Flower in data) {
            neighbors += Flower(
                ...
                abs(flower.PetalWidthCm - PetalWidthCm),
                flower.Species
            )
        }
...
fun main(args: Array<String>) {
    ...
    while (continueLoop) {
        ...
        val response = readLine()!!.lowercase(Locale.getDefault())
